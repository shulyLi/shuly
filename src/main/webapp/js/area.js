...
function shabi(i){
    alert( document.getElementById(s[i]).selectedIndex);
}
function _init_area(){  //初始化函数
    for(i=0;i<s.length-1;i++){
        $("#"+s[i]).attr("onchange","change("+(i+1)+");");
    }
    //$("#s_city").attr("onchange","startData();");
    change(0);
}
function _init_province(){
    ...
    with(ss){
        //options[0]=new Option("不选相当于不限之",opt0[0]);
        if(dsy.Exists(str)){
            ar = dsy.Items[str];
            for(i=0;i<ar.length;i++){
                options[length]=new Option(ar[i],str+'_'+i);
            }//end for
        }
    }
}